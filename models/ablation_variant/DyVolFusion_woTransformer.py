...
class LSTMResidualBranch(nn.Module):
    def __init__(self, enc_in: int, d_model: int, seq_len: int, pred_len: int):
        super().__init__()
        self.lstm = nn.LSTM(
            input_size=enc_in, hidden_size=d_model, num_layers=1, batch_first=True
        )
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.res_branch = LSTMResidualBranch(self.enc_in, self.d_model, self.seq_len, self.pred_len)
